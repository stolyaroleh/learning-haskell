...
tagWithBool :: (Applicative f) => (a -> f Bool) -> List a -> f (List (a, Bool))
tagWithBool p = sequence . map idAndBool
  where idAndBool a = lift2 (,) (pure a) (p a)

removeFalse :: List (a, Bool) -> List a
removeFalse = foldRight addIfTrue Nil
  where addIfTrue (a, b) acc = if b
                                  then a :. acc
                                  else acc
...
filtering ::
  ...
  -> List a
  -> f (List a)
filtering p l = removeFalse <$> tagWithBool p l
